...
const { store, fetchMyPlannings } = usePlanning();
...
const load = async () => {
  ...
  try {
    await fetchMyPlannings(search.value, currentPage.value);
  } finally {
  ...
  <div>
    <div class="mb-[22px] flex flex-wrap items-center justify-between gap-4">
      ...
      <div class="flex flex-wrap items-center gap-[10px]">
        ...
        <AppButton variant="primary" to="/planning/new">
    ...
    <p class="mb-4 text-[12px] text-gray-400">
      {{ store.pagination.count }} planning{{
        store.pagination.count > 1 ? "s" : ""
      }}
      personnel{{ store.pagination.count > 1 ? "s" : "" }}
    </p>
